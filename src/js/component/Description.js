...
export const Description = (props) => {
    ...
    let comp = props.data.map((data,i) => {
        // This is needed for the title. do no delete.
        const name = data.properties.name
        // Delete all non usable properties:
        const dataProps = {...data}

        const propertiesToDelete = ['name', 'created', 'edited', 'homeworld', 'url', 'films', 'pilots', "crew", "passengers"];
        ...
        <Container  key={"C"+i}>
            <Row className="border-bottom border-danger border-3 pb-5">
                ...
                <Col>
                    <h1 className="border-bottom mt-3">{name}</h1>
